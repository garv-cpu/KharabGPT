    ...
    <div className="bg-gradient-to-b from-white to-green-50 min-h-screen text-gray-800">
      ...
      <section className="max-w-6xl mx-auto px-4 sm:px-6 py-20 grid grid-cols-1 md:grid-cols-2 gap-10 items-center">
        <motion.div initial={{ opacity: 0, y: 50 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }}>
          ...
          <p className="text-base sm:text-lg mb-6">
            Fork AI helps you understand your food in seconds. Just snap a photo, and let our AI tell you the calories, ingredients, and health tips.
          </p>
      ...
      {showCamera && (
        <section className="px-4 sm:px-6 py-10 text-center">
          ...
          <div className="relative inline-block w-full max-w-md">
            <video ref={videoRef} autoPlay playsInline className="rounded-xl w-full shadow" />
          ...
          {imageURL && (
            <div className="mt-6">
              <img src={imageURL} alt="Captured food" className="w-full max-w-md mx-auto rounded-lg shadow" />
              <div className="mt-4 max-w-md mx-auto">
                {loading ? (
                  <div className="text-green-600 font-semibold text-center animate-pulse">Analyzing with AI...</div>
                ) : aiResult ? (
                  <div className="bg-white border border-green-200 rounded-lg shadow p-4 text-left">
                    <h3 className="text-lg font-semibold mb-2 text-green-700">🍽️ Nutrition Breakdown</h3>
                    <pre className="text-sm whitespace-pre-wrap text-gray-700">{aiResult}</pre>
                  </div>
                ) : null}
              </div>
            </div>
      ...
      {scans.length > 0 && (
        <section className="px-4 sm:px-6 py-12 bg-white">
          <h3 className="text-2xl font-semibold mb-6 text-center">Your Previous Scans</h3>
      ...
      <section className="py-20 px-4 sm:px-6 text-center">
        <motion.h2 className="text-3xl sm:text-4xl font-bold mb-6" initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} transition={{ duration: 0.6 }}>
